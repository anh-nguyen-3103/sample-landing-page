...
export const ClientItem = ({ item, onMouseEnter, onMouseLeave }: ClientItemProps) => {
  return (
    <div
      className="flex min-w-[400px] md:min-w-[800px] h-full bg-[#F7F7FA] p-6 min-h-[300px] shadow-md rounded-xl overflow-hidden flex-col justify-between"
      onMouseEnter={onMouseEnter}
      onMouseLeave={onMouseLeave}
    >
      <div className="flex flex-col gap-4">
        <Quote fill="#7488E233" className="rotate-180 -rotate-z-90" />
        <p className="text-md md:text-xl font-medium text-gray-500">{item.notes}</p>
      </div>
