...
interface JobListWrapperProps {
  initialJobs: PaginatedDocs<Job>
}

export function JobListWrapper({ initialJobs }: JobListWrapperProps) {
  return <JobList jobs={initialJobs} />
}

function JobList({ jobs }: { jobs: PaginatedDocs<Job> }) {
  ...
  return (
    <div className="flex flex-col w-full h-full px-4 sm:px-8 md:px-12 py-8 md:py-12 gap-6 md:gap-8">
      <div className="flex justify-between items-center w-full">
        <h1 className="text-black text-3xl font-bold">List Job</h1>
      </div>
      {data.docs.map((job) => (
